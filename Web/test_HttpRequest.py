...
def test_http_request_1():
    ...
    request = httpRequest.getNextHttpRequest(noTagsHeader, logging)
...
def test_http_request_host():
    ...
    request = httpRequest.getNextHttpRequest(oneTagsHeader, logging)
...
def test_http_request_getFirstLine_methodGet():
    ...
    method, _, _ = httpRequest.getFirstLine(socket, logging)
    assert method == "GET"
...
def test_http_request_getFirstLine_resource():
    socket = fakeSocket(b'GET /bank/Main-page HTTP/1.1\r\n')
    _, resource, _ = httpRequest.getFirstLine(socket, logging)
