...
def test_verifying_cursor_class():
    ...
    assert isinstance(cursor_id1, FakeCursor)
...
def test_accessing_connections_tuple():
    ...
    assert isinstance(psql_conn.used_conns[id1.value()], tuple)
...
def test_verifying_if_the_second_tuple_item_is_not_none():
    ...
    assert psql_conn.used_conns[id1.value()][1] is not None
